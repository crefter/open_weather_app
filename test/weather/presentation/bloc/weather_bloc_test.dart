...
void main() {
  group('weather bloc tests', () {
    ...
    test(
        'should state is loaded when added load event if get current weather is successful and city length >= 2',
        () async {
    ...
    test('should do nothing if city length < 2', () async {
      weatherBloc.add(const WeatherEvent.load('A'));
      await Future.delayed(const Duration(microseconds: 50));
      final actual = weatherBloc.state;
      expect(actual, const WeatherState.initial());
    });
  });
}
